...
hierarchical_clusters <- cutree(hc, k = 5)  # Use k=5 for fair comparison
...
clustering_results <- data.frame(
  ...
  Num_Clusters = c(5, num_spectral_clusters, 5),
...
dunn_indices <- data.frame(
  ...
  Dunn_Index = c(
    ...
    dunn(clusters = cutree(hc, k=5), Data = activity_matrix)
  )
)
